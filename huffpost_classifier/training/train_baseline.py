"""Training routine for the baseline model."""
...
from huffpost_classifier.data.datamodule_baseline import BaselineDataModule
from huffpost_classifier.models.baseline_module import BaselineClassifier
...
def train_baseline(cfg: Any) -> Dict[str, Path]:
    """Train the baseline model and return artifact paths."""
    artifacts_dir = Path(cfg.output_dir)
    baseline_dir = ensure_dir(artifacts_dir / "baseline")
    plots_dir = ensure_dir(Path(cfg.paths.plots_dir) / cfg.run_name)

    datamodule = BaselineDataModule(cfg)
    datamodule.setup()
    ...
    model = BaselineClassifier(
        cfg, vocab_size=len(vocab), num_classes=num_classes, class_weights=class_weights
    )
    ...
    best_model = BaselineClassifier.load_from_checkpoint(
        str(best_checkpoint),
        cfg=cfg,
    ...
    torch.save(best_model.model.state_dict(), baseline_dir / "model.pt")

    vocab.save(baseline_dir / "vocab.json")
    with (baseline_dir / "label_map.json").open("w", encoding="utf-8") as file:
        json.dump(label_map, file)
    with (baseline_dir / "model_config.json").open("w", encoding="utf-8") as file:
        json.dump(
            {
                "vocab_size": len(vocab),
                "embed_dim": cfg.model.embed_dim,
